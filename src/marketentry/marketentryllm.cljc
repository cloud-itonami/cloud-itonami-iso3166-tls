(ns marketentry.marketentryllm
  ...
  the TLS public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向け提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-serve-registration?=" (:has-serve-registration? e)
                        " uses-local-works-procedure?=" (:uses-local-works-procedure? e)
                        " tender-value=" (:tender-value e)
     ...
     :confidence (if (and e
                          (or (not (:requires-serve-registration? e))
                              (:has-serve-registration? e))
                          (registry/local-works-procedure-satisfied? e))
                   0.9 0.3)}))
