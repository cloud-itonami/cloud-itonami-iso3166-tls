(ns marketentry.registry
  ...
  `local-works-procedure-satisfied?` is a pure re-derivation of the
  SAME test `marketentry.governor`'s flagship check applies (value
  ceiling + fraud/corruption bar under Decreto-Lei n.º 2/2010), exposed
  here for reuse outside the governor (e.g. by
...
(defn local-works-procedure-satisfied?
  "Does `engagement` independently satisfy the local-works-procedure
  gate -- i.e. either it does not itself invoke Decreto-Lei n.º
  2/2010's decentralized civil-works track, or it does and its own
  `:tender-value` is within the $250,000 ceiling AND it is not
  fraud/corruption-sanctioned? Pure re-derivation of the same test
  `marketentry.governor`'s flagship check applies."
  [{:keys [uses-local-works-procedure? tender-value fraud-or-corruption-sanctioned?]}]
  (or (not uses-local-works-procedure?)
      (and (or (nil? tender-value) (<= tender-value 250000))
           (not (true? fraud-or-corruption-sanctioned?)))))
