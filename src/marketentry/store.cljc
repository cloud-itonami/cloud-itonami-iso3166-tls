(ns marketentry.store
  "SSoT for the TLS market-entry compliance actor, behind a `Store`
  ...
  `:requires-serve-registration?`/`:has-serve-registration?` ground the
  SERVE commercial-registration check (Decreto-Lei n.º 16/2017). The
  flagship check (Decreto-Lei n.º 2/2010) needs THREE fields, not the
  usual two-boolean-pair shape: `:uses-local-works-procedure?` (does
  this engagement invoke the decentralized civil-works track at all --
  most engagements do not), `:tender-value` (the underlying public
  procurement contract's own value in USD, kept deliberately distinct
  from `:base-fee`/`:claimed-fee`, which are THIS ACTOR's own service
  fee for market-entry assistance, not the client's tender value), and
  `:fraud-or-corruption-sanctioned?` (Artigo 2.º §4's admissibility
  bar). See `marketentry.governor` for the corresponding flagship-check
  docstring."
...
(defn demo-data
  ...
  the 6 engagements exercises a distinct governor path:
    eng-1 -- clean (commit path once assessed/drafted/submitted),
             invokes the local-works procedure within scope
    eng-2 -- serve-registration-missing (requires but lacks SERVE
             commercial registration)
    eng-3 -- local-works-procedure-ineligible, the VALUE-CEILING branch
             (tender-value exceeds Decreto-Lei n.º 2/2010's $250,000
             scope)
    eng-4 -- local-works-procedure-ineligible, the FRAUD/CORRUPTION
             branch (sanctioned under Artigo 2.º §4)
    eng-5 -- engagement-fee-mismatch (claimed-fee != base + months x
             rate)
    eng-6 -- clean, does NOT invoke the local-works procedure at all
             (a pure public-tender engagement outside that decree's
             scope -- the conditional branch of the flagship check
             still commits clean)"
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Dili Uma Kmanek Lda" :portal "Aprovisionamento (Decreto-Lei n.º 10/2005) filing"
             :base-fee 220000 :monthly-rate 15000 :monitoring-months 6
             :claimed-fee 310000.0
             :requires-serve-registration? true :has-serve-registration? true
             :uses-local-works-procedure? true :tender-value 180000 :fraud-or-corruption-sanctioned? false
             :drafted? false :submitted? false
             :jurisdiction "TLS" :status :intake}
    "eng-2" {:id "eng-2" :operator "Baucau Construção Unipessoal" :portal "Aprovisionamento (Decreto-Lei n.º 10/2005) filing"
             :base-fee 220000 :monthly-rate 15000 :monitoring-months 6
             :claimed-fee 310000.0
             :requires-serve-registration? true :has-serve-registration? false
             :uses-local-works-procedure? false :tender-value nil :fraud-or-corruption-sanctioned? false
             :drafted? false :submitted? false
             :jurisdiction "TLS" :status :intake}
    "eng-3" {:id "eng-3" :operator "Maliana Obras Públicas SA" :portal "Aprovisionamento (Decreto-Lei n.º 10/2005) filing"
             :base-fee 220000 :monthly-rate 15000 :monitoring-months 6
             :claimed-fee 310000.0
             :requires-serve-registration? true :has-serve-registration? true
             :uses-local-works-procedure? true :tender-value 340000 :fraud-or-corruption-sanctioned? false
             :drafted? false :submitted? false
             :jurisdiction "TLS" :status :intake}
    "eng-4" {:id "eng-4" :operator "Same Infraestrutura Lda" :portal "Aprovisionamento (Decreto-Lei n.º 10/2005) filing"
             :base-fee 220000 :monthly-rate 15000 :monitoring-months 6
             :claimed-fee 310000.0
             :requires-serve-registration? true :has-serve-registration? true
             :uses-local-works-procedure? true :tender-value 90000 :fraud-or-corruption-sanctioned? true
             :drafted? false :submitted? false
             :jurisdiction "TLS" :status :intake}
    "eng-5" {:id "eng-5" :operator "Ainaro Comércio Geral" :portal "Aprovisionamento (Decreto-Lei n.º 10/2005) filing"
             :base-fee 220000 :monthly-rate 15000 :monitoring-months 6
             :claimed-fee 999000.0
             :requires-serve-registration? true :has-serve-registration? true
             :uses-local-works-procedure? true :tender-value 100000 :fraud-or-corruption-sanctioned? false
             :drafted? false :submitted? false
             :jurisdiction "TLS" :status :intake}
    "eng-6" {:id "eng-6" :operator "Dili Trade Partners SA" :portal "Aprovisionamento (Decreto-Lei n.º 10/2005) filing"
             :base-fee 150000 :monthly-rate 10000 :monitoring-months 4
             :claimed-fee 190000.0
             :requires-serve-registration? true :has-serve-registration? true
             :uses-local-works-procedure? false :tender-value nil :fraud-or-corruption-sanctioned? false
             :drafted? false :submitted? false
             :jurisdiction "TLS" :status :intake}}})
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-serve-registration? has-serve-registration?
                               uses-local-works-procedure? tender-value fraud-or-corruption-sanctioned?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-serve-registration?)           (assoc :engagement/requires-serve-registration? requires-serve-registration?)
    (some? has-serve-registration?)                (assoc :engagement/has-serve-registration? has-serve-registration?)
    (some? uses-local-works-procedure?)            (assoc :engagement/uses-local-works-procedure? uses-local-works-procedure?)
    (some? tender-value)                           (assoc :engagement/tender-value tender-value)
    (some? fraud-or-corruption-sanctioned?)        (assoc :engagement/fraud-or-corruption-sanctioned? fraud-or-corruption-sanctioned?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-serve-registration? :engagement/has-serve-registration?
   :engagement/uses-local-works-procedure? :engagement/tender-value :engagement/fraud-or-corruption-sanctioned?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-serve-registration? (boolean (:engagement/requires-serve-registration? m))
     :has-serve-registration? (boolean (:engagement/has-serve-registration? m))
     :uses-local-works-procedure? (boolean (:engagement/uses-local-works-procedure? m))
     :tender-value (:engagement/tender-value m)
     :fraud-or-corruption-sanctioned? (boolean (:engagement/fraud-or-corruption-sanctioned? m))
