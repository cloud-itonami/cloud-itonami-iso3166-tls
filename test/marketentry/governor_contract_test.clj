(ns marketentry.governor-contract-test
  ...
  This is the TLS actor's SEVEN-check governor. The flagship check
  (`local-works-procedure-ineligible`) is itself a genuine TWO-condition
  test (value-ceiling breach OR fraud/corruption sanction), reflecting
  Decreto-Lei n.º 2/2010's own Artigo 2.º -- see `marketentry.governor`
  for the full discussion of which of that article's six admissibility
  conditions this governor does, and deliberately does not, enforce as
  a hard gate."
...
(deftest clean-intake-auto-commits
  (let [[db actor] (fresh)
    ...
    (is (= "Dili Uma Kmanek Lda" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest serve-registration-missing-is-held-and-unoverridable
  (testing "missing SERVE commercial registration -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (some #{:serve-registration-missing} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest local-works-procedure-value-ceiling-is-held-and-unoverridable
  (testing "tender-value exceeds Decreto-Lei n.º 2/2010's $250,000 ceiling -> HARD hold (flagship, value branch)"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:local-works-procedure-ineligible} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest local-works-procedure-fraud-sanction-is-held-and-unoverridable
  (testing "fraud/corruption sanctioned (Artigo 2.º §4) -> HARD hold (flagship, fraud branch)"
    (let [[db actor] (fresh)
...
(deftest engagement-not-using-local-works-procedure-still-commits-end-to-end
  (testing "eng-6 does NOT itself invoke the local-works procedure -- the conditional branch of the flagship check still commits clean"
    (let [[db actor] (fresh)
          _ (assess! actor "t9pre" "eng-6")
          _ (draft! actor "t9pre" "eng-6")
          r1 (exec-op actor "t9" {:op :filing/submit :subject "eng-6"} operator)
          r2 (approve! actor "t9")]
